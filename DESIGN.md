...
- **Empty state.** An Inter headline ("What deck are we making?") over one short line of Muted Ink body, then Write the brief and Open a deck (the drop hint lives in its tooltip). No illustration: the chrome's only pictures are Lucide icons. The last-session offer is one row: a History icon, the deck title with its slide count, then Discard (ghost) and Pick it up (primary) inline; white, 5% border, `shadow-float`.
