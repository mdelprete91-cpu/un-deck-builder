...
- **The generating pill** (Mario, 25 Sep 2026): while the first slide is being written the empty stage shows the slide bar's own pill, "Generating…" in Ink with its spinner, the button's measurements and no tint (a status, not a control), centred on the stage, with an aurora behind it: `.gen-pill::before` in globals.css, a conic gradient of the four chart colours (Giga green and blue, UNICEF red and yellow), 8px out and blurred 18px at 60%, turning once every eight seconds through a registered `--gen-angle` property while it breathes (opacity 40 to 70%, scale 1 to 1.025, three seconds, ease-in-out), static under reduced motion. When the first slide lands, the pill travels to the slide bar's place and becomes it: the pill's place is measured right before the first append and the bar, mounted where it belongs, flies in from there with a FLIP on transform alone (640ms ease-out-expo, Web Animations API, compositor-driven so the first slide being drawn cannot make it stutter; a view transition was tried first and the browser skipped it); the bar skips its own float-in while busy, stays mounted through every landing slide (one key for the whole generation, so the aurora never restarts) and keeps it until the deck is done. It is the one glow in the chrome and the one exception to "nothing glows": it means the deck is being written and nothing else.
...
- **Sheet wizard** (`components/SheetWizard.tsx`). The image picker's box (576px, 24px radius, `shadow-float`, the scrim, `pop-in`) walking the questions a spreadsheet raised, one per step: the file name as the title with the sheet glyph in Giga Blue; the model's summary in Muted Ink; a 13px Faint Ink "Question 2 of 5 · 1 answered" line over a 4px Canvas-2 track with a Giga Blue fill; the question at 16px medium, its "why" at 13px Muted Ink; options as 40px rows with a 16px circle (single) or 4px-radius square (multi) at the left, Mist on hover, Giga tint with the accent-filled control and a white tick when picked; a text question is the standard textarea. Footer: Skip as ghost on the left, Back (secondary) and Next (primary, arrow) on the right; on the last question the primary says Generate (tick) when the deck follows, Next file with more files to ask, Done when reopened from the row; ⌘↵ advances, Esc closes and keeps the answers without generating. While the material is read the body is a Canvas-2 skeleton in the shape of a question, and the only control is "Skip the questions". It opens on the Generate press, never when a file lands; the brief alone can open it too, titled "Your brief".
- **Edit with AI** (`components/EditWithAiModal.tsx`, Mario, 25 Sep 2026). The bar's accent button opens a dialog instead of an input inside the bar: the layout switcher's box (max-w-4xl), a "What should change?" textarea, then Layout as a three-column grid of previews: this slide rendered in its current layout first ("Current") and in each layout of its family from `switchTargets` that hold every item of the slide (one that would drop items is not offered, Mario, 26 Sep 2026), label and note under each, a red "Text shrinks" badge where it applies, the picked card ringed in Ink (a name says nothing, the picture does: Mario, 25 Sep 2026), and a Canvas-2 note that the AI never edits photos or images (Image in the bar does). The primary button says Rewrite when there is an instruction and Apply layout when only the layout changed; ⌘↵ submits, Esc closes.
- **Deck name** (`components/DeckName.tsx`): the name in the toolbar with a 14px Lucide pencil after it, Faint Ink at rest and Ink on hover, so it reads as an editable name (Mario, 26 Sep 2026); a click turns it into the standard input.
- **Present** (`components/Presenter.tsx`): an icon-only secondary Button (36px, Lucide Play) after Download, slides only, so the bar stays compact. It requests full screen inside the click, then shows the built HTML deck in a frame from the active slide; the frame's surround is #111 (the presentation, not the chrome) with a faint ✕ top right; leaving full screen ends it.
- **Add slide** heads the thumbnail column in a fixed 57px row with the toolbar's hairline under it, outside the scrolling list, so the thumbnails never slide under it; a new slide lands right after the selected one.
