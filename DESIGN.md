...
- **Empty state.** An Inter headline ("What deck are we making?") over one line of Muted Ink body, then Write the brief and Open a deck. No illustration: the chrome's only pictures are Lucide icons. The last-session offer is one row: a History icon, the deck title with its slide count, a one-line note, then Discard (ghost) and Pick it up (primary) inline; white, 5% border, `shadow-float`.
