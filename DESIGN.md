...
- **Image picker.** One header on every screen (back chevron where there is a screen behind, the title, the close), 24px radius, `shadow-float`. The choose screen is two tiles and a ghost link. The country map screen puts the preview on the left at up to 440px tall and three stacked settings on the right (Country as a bordered select-like button, Show and Style as segmented controls) with one status line under them: the count, or what is rendering, or what went wrong. Footer: "Use a screenshot instead" (ghost), Cancel (ghost), Use this map (primary).
