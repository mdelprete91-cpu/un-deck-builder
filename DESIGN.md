...
- **Edit with AI** (`components/EditWithAiModal.tsx`, Mario, 25 Sep 2026). The bar's accent button opens a dialog instead of an input inside the bar: the image picker's box, a "What should change?" textarea, a Layout radiogroup (Keep the current layout, then the family's alternatives from `switchTargets` with their note and a red "2 of 4 items" badge where one shows fewer), and a Canvas-2 note that the AI never edits photos or images (Image in the bar does). The primary button says Rewrite when there is an instruction and Apply layout when only the layout changed; ⌘↵ submits, Esc closes.
