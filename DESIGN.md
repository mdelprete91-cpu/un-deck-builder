...
- **Slide bar.** A white pill (10px padding, 8px gaps, `shadow-float`) inside a 1.5px conic-gradient ring that runs the full spectrum (Giga Blue, cyan, violet, pink, amber) and turns once every six seconds (still under reduced motion), centered under the slide. Every action is a Button: "Edit with AI" in the accent variant, then only the actions that apply to this slide (Element, Data, Image), then a slightly wider gap and the icon-only pair, Duplicate and Delete (danger). While the AI works the bar shows one accent pill with a spinner. The instruction field, once open, is a 36px bare input followed by Regenerate (primary, enabled when there is text) and a ghost close.
