...
- **One button spec.** Every chrome button is 36px tall (`h-9`), Inter 14px medium, copied from chatgpt.com/library on 15 Sep 2026: 12px side padding (`px-3`) with a 14px Lucide icon and a 6px gap when there is an icon beside the label (chevrons stay 16), 16px padding (`px-4`) when it is text only, and icon-only pills are 36px squares with a 16px icon. The same spec on the toolbar (Undo, Redo, Upload, Download), the slide bar, the sidebar (Add slides, How it works, Delete deck), the filmstrip (Add slide) and the modals. Nothing is 12px or 13px, nothing is 40px tall.
