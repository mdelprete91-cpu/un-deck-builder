...
- **Slide bar.** A white pill (10px padding, 8px gaps, 5% hairline, `shadow-float`) centered under the slide. It morphs between its modes rather than swapping: the width transitions over 320ms (ease-out-expo) from the measured content, and the new controls fade up 60ms later, 40ms apart (`.bar-morph`, `.bar-mode`; static under reduced motion). Every action is a Button: "Edit with AI" in the accent variant, wearing the 1.5px conic-gradient ring (`.gradient-ring`, an inset overlay so the button keeps its 36px) that runs the full spectrum (Giga Blue, cyan, violet, pink, amber) and turns once every six seconds (still under reduced motion), then only the actions that apply to this slide (Element, Data, Image), then a slightly wider gap and the icon-only pair, Duplicate and Delete (danger). While the AI works the bar shows one accent pill with a spinner. The instruction field, once open, is a 36px bare input followed by Regenerate (primary, enabled when there is text) and a ghost close.
