...
- **The generating stage** (Mario's Figma, 25 Sep 2026): while the first slide is being written the empty stage shows a slide-shaped slab in Surface (16px radius, light hairline) with a soft aurora around it, `.gen-stage` in globals.css: a conic gradient of the four chart colours (Giga green and blue, UNICEF red and yellow) blurred 40px, turning once every eight seconds through a registered `--gen-angle` property while it breathes (opacity 40 to 70%, scale 1 to 1.025, three seconds, ease-in-out), static under reduced motion, and clipped inside the stage column (112px of room for the blur, more than it reaches) so it never reaches the sidebars. It is the one glow in the chrome and the one exception to "nothing glows": it means the deck is being written and nothing else.
