...
- **Selects are settings rows** (`components/Select.tsx`): the label on the left, the current value and a chevron on the right as a borderless text button, a 5% hairline under the row. The menu is a white card, 16px radius, `shadow-menu` (ChatGPT's: a soft 8px drop plus a 1px outline at 62%), 220px minimum, 36px options at 10px radius with a 4% wash on hover and on the selected one, which also carries a tick on the right. The Download menu uses the same card, with bare labels (PDF, HTML deck, PowerPoint) and one tag on HTML deck, "Re-upload to edit" in the accent tint, because that is the file Upload takes back. Never a native `<select>`: its popup is the one thing the chrome cannot style.
...
- **Toolbar.** White band with bottom hairline. The deck name on the left (`components/DeckName.tsx`): Inter 500 Ink at 16px in a 36px borderless pill that washes Mist on hover and turns into a 36px hairline input on click, Giga Blue border and soft ring on focus. Everything else sits right-aligned in one row of Buttons: Undo, Redo, Upload (secondary), Download (primary).
...
- **Slide bar.** A white pill (10px padding, 8px gaps, 5% hairline, `shadow-float`) centered under the slide. It morphs between its modes rather than swapping: the width transitions over 320ms (ease-out-expo) from the measured content, and the new controls fade up 60ms later, 40ms apart (`.bar-morph`, `.bar-mode`; static under reduced motion). Every action is a Button: "Edit with AI" in the ring variant (white, Ink text, no fill, no hairline), wearing the 1.5px conic-gradient ring (`.gradient-ring`, an inset overlay so the button keeps its 36px, with the same gradient blurred 7px at 55% underneath so the ring reads as diffused light; the one glow in the chrome, Mario's call) that runs the full spectrum (Giga Blue, cyan, violet, pink, amber) and turns once every six seconds (still under reduced motion), then only the actions that apply to this slide (Element, Data, Image), then a slightly wider gap and the icon-only pair, Duplicate and Delete (danger). While the AI works the bar shows one accent pill with a spinner. The instruction field, once open, is a 36px bare input followed by Regenerate (primary, enabled when there is text) and a ghost close.
- **Image picker.** A settings dialog: a Canvas rail on the left with the two sections as rows (Upload, Maps; the current one is the bordered pill), the section on the right under a title with the close at its right and a hairline, 24px radius, `shadow-float`. Country map is three settings rows (Country, Show, Style: label left, value and chevron right, hairline between) with one status line under them, and the preview on the right inside a fixed 4:3 frame on Canvas-2, the map contained in it whatever the slot's shape, so the dialog never changes shape between layouts. Footer: Cancel (ghost), Use this map (primary).
- **Empty state.** An Inter headline ("What deck are we making?") over one short line of Muted Ink body, then Write the brief and Open a deck (the drop hint lives in its tooltip). No illustration: the chrome's only pictures are Lucide icons. The last-session offer is one row: a History icon, the deck title with its slide count, then Discard (ghost) and Pick it up (primary) inline; a full pill (`rounded-full`), white, 5% border, `shadow-float`.
