...
- **One button spec.** Every chrome button is 36px tall (`h-9`), Inter 14px medium, with 14px side padding (`px-3.5`) and a 16px Lucide icon before the label when it has one (`gap-1.5`); icon-only pills are 36px squares. The same spec on the toolbar (Undo, Redo, Upload, Download), the slide bar, the sidebar (Add slides, How it works, Delete deck), the filmstrip (Add slide) and the modals. Nothing is 12px or 13px, nothing is 40px tall.
