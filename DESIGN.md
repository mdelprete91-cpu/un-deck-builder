...
- **The generating stage** (Mario's Figma, 25 Sep 2026): while the first slide is being written the stage shows a slab exactly where and how big the slide will be (the toolbar's 56px above, the canvas's 24/40px padding, the slide's ratio and 12px corners; `.gen-fit` is a size container and `.gen-stage` the largest box of the ratio that fits it, the same maths as SlideFrame) in Surface with a light hairline, "Generating…" in Muted Ink at its centre, and a tight aurora around it, `.gen-stage::before` in globals.css: a conic gradient of the four chart colours (Giga green and blue, UNICEF red and yellow), 6px out and blurred 16px, so it fades inside the canvas padding, turning once every eight seconds through a registered `--gen-angle` property while it breathes (opacity 40 to 70%, scale 1 to 1.025, three seconds, ease-in-out), static under reduced motion, and clipped to the canvas so it never reaches the sidebars. It is the one glow in the chrome and the one exception to "nothing glows": it means the deck is being written and nothing else.
