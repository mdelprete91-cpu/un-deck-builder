...
- **Reference files**: up to 6 files and 3 MB per brief. A PDF too big to send whole goes as its text (the chip says "text only"). The deck is about the material: the brief decides length, angle and which organisations to feature.
...
- **Change layout**: "Layout" in the slide bar opens a picker where the top row is the slide's own text already written into three or four other layouts of the template, chosen by the AI for holding it (a badge says "All 4 items" or "2 of 4 items" when a layout holds fewer), so a pick is instant and undoable. Below it, every other layout with placeholder content: a pick there sends the slide to the AI to be moved, text kept.
- **The brief sets the length and the structure**: "six slides", "a 20-page deck", "10 diapositive" is honoured exactly, and "one slide per objective, titled with the objective" is followed to the letter, no item dropped; say "same layout" if the series should repeat one.
