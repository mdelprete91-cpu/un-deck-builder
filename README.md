...
- **Spreadsheets**: an Excel file becomes one table per sheet (dates and percentages as you see them in Excel, 200 rows a sheet), and a card under its chip asks what the deck should draw from it: "connected schools by region as a chart" lands on the chart and stat slides with the cell values as written. Leave it empty and the sheet is read against the brief alone.
