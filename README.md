...
- **Product tour**: "How it works" in the sidebar walks the brief, the Chapters toggle and Generate, and, once a deck exists, the canvas, the slide bar, Add slides and Download. It never starts on its own. Under the button, after a generation, a small card with a party popper says what it cost (Haiku list price) and how long it took; the session total is in its tooltip.
