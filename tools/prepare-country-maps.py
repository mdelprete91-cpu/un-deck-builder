...
def main() -> int:
    ...
    for path in files:
        ...
        note = ""
        empty = box is None
        if empty:  # no schools plotted: keep the export centred as-is
            box = (0, 0, im.size[0], im.size[1])
            note = "  (NO DOTS in the export)"
        entries.append((path.stem, slugify(path.stem), empty))
...
def write_list(entries: list[tuple[str, str, bool]]) -> None:
    """
    Emit the TypeScript list alongside the images, so the two cannot drift —
    the failure mode partners.ts warns about, where a name stops matching a file.
    """
    rows = "\n".join(
        f'  {{ name: "{name}", slug: "{slug}"{", empty: true" if empty else ""} }},'
        for name, slug, empty in entries
    )
    empties = [name for name, _, empty in entries if empty]
    dest = Path(__file__).resolve().parent.parent / "lib" / "slides" / "country-maps.ts"
    dest.write_text(
        f'''/**
 ...
 * marks a country whose export has no schools plotted at all ({", ".join(empties) or "none"}
...
'''
    ...
    print(f"wrote {dest.relative_to(dest.parent.parent.parent)} ({len(entries)} countries)")
